...
class CTBService {
  constructor(options = {}) {
    ...
    this.collector = new CTBDataCollector(this.requestsPerSecond, this.concurrentRequests);
  ...
  async collectAndSaveData() {
    try {
      ...
      const { routeStops, stopRoutesMap } = CTBDataProcessor.processRouteStopResults(routeStopResults);

      // Step 3: Collect stop details
      const stopIds = Object.keys(stopRoutesMap);
      console.log(`Found ${stopIds.length} unique stops`);

      const stopDetailsResults = await this.collector.collectAllStopDetails(stopIds);
      const successfulStops = CTBDataProcessor.processStopDetailsResults(stopDetailsResults);
      ...
      for (const { stopId, data } of successfulStops) {
        const enrichedStopData = CTBDataProcessor.enrichStopWithRoutes(data, stopRoutesMap, stopId);
      ...
      for (const [route, stops] of Object.entries(routeStops)) {
        const enrichedRouteData = CTBDataProcessor.createEnrichedRouteData(route, routeStops, successfulStops);
