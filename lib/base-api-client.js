...
class BaseApiClient {
  ...
  /**
   * Fetch JSON data from URL with caching, rate limiting, and professional retries
  ...
  async fetchJson(url) {
    ...
    try {
      ...
      return await pRetry(
        ...
        },
        {
          ...
          onFailedAttempt: error => {
            ...
            const label = statusCode || error.code || 'Network Error';
            console.warn(
              `⚠️ Attempt ${error.attemptNumber} failed for ${url} (${label}). ${error.retriesLeft} retries left...`
            );
          },
          shouldRetry: error => {
            const statusCode = error.response?.status;

            // Retry transient upstream/server/network failures
            if (
              statusCode === 408 ||
              statusCode === 425 ||
              statusCode === 429 ||
            ...
            // Retry when there is no HTTP response at all
            if (!statusCode) {
              return true;
            }

            // Treat intermittent 403 from this upstream as transient
            if (statusCode === 403) {
              return true;
            }
      ...
      throw new NetworkError(`Failed to fetch ${url} after retries`, {
        url,
