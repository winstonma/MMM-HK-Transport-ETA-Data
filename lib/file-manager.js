...
class CTBFileManager {
  ...
  async saveAllRoutes(allRoutesData) {
    try {
      ...
      console.log(`Original size: ${JSON.stringify(allRoutesData).length} characters`);
      console.log(`Compact size: ${JSON.stringify(compactData).length} characters`);
      console.log(`Reduction: ${Math.round(((JSON.stringify(allRoutesData).length - JSON.stringify(compactData).length) / JSON.stringify(allRoutesData).length) * 100)}%`);
      return true;
    } catch (error) {
