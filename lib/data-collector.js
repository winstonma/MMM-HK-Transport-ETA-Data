...
const { CIRunProgress } = require('./ci-progress');
...
class CTBDataCollector extends BaseDataCollector {
  ...
  async collectRoutes() {
    const spinner = (await import('ora')).default('Collecting CTB routes...').start();
  ...
  async collectAllRouteStops(routes) {
    ...
    const spinner = (await import('ora')).default(`Collecting route stops (0/${total})`).start();
    const progress = new CIRunProgress(total, 'Route Collection', 30000);
    ...
    for (let i = 0; i < total; i += limit) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map(route => this.collectRouteStops(route))
      );
      ...
      spinner.text = `Collecting route stops (${completed}/${total})`;
      progress.track(completed, 'Collecting route stops...');
    }

    spinner.succeed(`Collected route stops for ${total} routes`);
    return results;
  }
  ...
  async collectOptimizedStopDetails(stopIds, stopRoutesMap) {
    ...
    spinner.stop(); // Stop the first spinner before starting the nested one
    const existingAllStops = await this.fetchExistingAllStops();

    // Restart spinner after fetching
    spinner.start();
...
class KMBDataCollector extends BaseDataCollector {
  ...
  async collectRoutes() {
    const spinner = (await import('ora')).default('Collecting KMB routes...').start();
  ...
  async collectAllRouteStopsData() {
    const spinner = (await import('ora')).default('Collecting all KMB route stops...').start();
    try {
  ...
  async collectStopDetailsForStops() {
    const spinner = (await import('ora')).default('Collecting all KMB stops...').start();
    try {
