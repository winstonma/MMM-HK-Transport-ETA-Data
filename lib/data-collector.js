...
class CTBDataCollector {
  ...
  async fetchExistingAllStops() {
    const spinner = ora('Fetching existing allstops.json from GitHub...').start();
    try {
      const url =
      ...
      spinner.warn('Could not fetch existing allstops.json, will collect all data fresh');
      return {};
    }
  }
  ...
  async collectAllRouteStops(routes) {
    ...
    const spinner = ora(`Collecting route stops (0/${routeList.length})`).start();
  ...
  async collectAllStopDetails(stopIds) {
    const spinner = ora(`Collecting stop details (0/${stopIds.length})`).start();
  ...
  async collectOptimizedStopDetails(stopIds, stopRoutesMap) {
    const spinner = ora(`Optimizing stop details collection for ${stopIds.length} stops...`).start();
