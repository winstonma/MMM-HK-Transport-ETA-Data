...
class BaseDataCollector {
  ...
  /**
   * Lazily initialize and return the shared ora spinner instance.
   * Imports ora on first call and caches the instance on this.spinner.
   * @returns {Promise<import('ora').Ora>} The shared spinner instance
   */
  async getSpinner() {
    if (!this.spinner) {
  ...
  async collectAllStopDetails(stopIds) {
    ...
    spinner.start(`Collecting stop details (0/${stopIds.length})`);

    const progress = new CIRunProgress(stopIds.length, 'Stop Details Collection', 30000);
    ...
    const results = await this.apiClient.processWithConcurrency(
      ...
      async stopId => {
        ...
        progress.track(completed, 'Collecting stop details...');
        return result;
      }
    );
