...
class KMBService {
  constructor(options = {}) {
    ...
    this.collector = new KMBDataCollector(
      this.requestsPerSecond,
      this.concurrentRequests
    );
    this.fileManager = new KMBFileManager(this.baseDir);
  }
  ...
  async collectAndSaveData() {
    try {
      ...
      for (const stopId of stopIds) {
        ...
        if (stopData) {
          const enrichedStopData = KMBDataProcessor.enrichStopWithRoutes(
            stopData,
            stopRoutesMap,
            stopId
          );
      ...
      for (const [stopId, stopData] of Object.entries(allStopsData)) {
        await this.fileManager.saveStopData(stopId, stopData);
      }
      ...
      for (const [route] of Object.entries(routeStops)) {
        const enrichedRouteData = KMBDataProcessor.createEnrichedRouteData(
          route,
          routeStops,
          successfulStops
        );
